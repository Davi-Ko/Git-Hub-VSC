...
void syncWithPeer(String command) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpCode > 0) {
            ...
            Serial.println("Sync successful with Relay 1: " + command);
        } else {
            isConnectedToPeer = false;
            Serial.println("Sync failed with Relay 1");
        }
        http.end();
    }
}
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpCode == 200) {
            ...
            if (payload.indexOf("ON") != -1 && !relayState) {
                ...
                Serial.println("Synced ON from Relay 1");
            } else if (payload.indexOf("OFF") != -1 && relayState) {
                relayState = false;
                digitalWrite(relayPin, HIGH);
                Serial.println("Synced OFF from Relay 1");
            }
        } else {
            isConnectedToPeer = false;
        }
...
void setup(){
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\nConnection Success!");
        Serial.print("Connected to Relay 1 with IP: ");
        Serial.println(WiFi.localIP());
        isConnectedToPeer = true;
    } else {
        Serial.println("\nFailed to connect to Relay 1. Operating in standalone mode.");
        isConnectedToPeer = false;
    }
    ...
    server.on("/", HTTP_GET, [](){
        ...
        String response = "Relay 2 Status: " + status + "\nPeer Connection: " + peerStatus;
    ...
    server.on("/ON", HTTP_GET, [](){
        relayState = true;
        digitalWrite(relayPin, LOW);
        server.send(200, "text/plain", "Relay 2 power: ON");
    ...
    server.on("/OFF", HTTP_GET, [](){
        relayState = false;
        digitalWrite(relayPin, HIGH);
        server.send(200, "text/plain", "Relay 2 power: OFF");
    ...
    server.on("/ON_SYNC", HTTP_GET, [](){
        relayState = true;
        digitalWrite(relayPin, LOW);
        server.send(200, "text/plain", "Relay 2 synced: ON");
    ...
    server.on("/OFF_SYNC", HTTP_GET, [](){
        relayState = false;
        digitalWrite(relayPin, HIGH);
        server.send(200, "text/plain", "Relay 2 synced: OFF");
...
void loop() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        if (isConnectedToPeer) {
            Serial.println("Lost connection to Relay 1! Trying to reconnect...");
            isConnectedToPeer = false;
        }
        ...
        isConnectedToPeer = true;
        Serial.println("Reconnected to Relay 1!");
    }
